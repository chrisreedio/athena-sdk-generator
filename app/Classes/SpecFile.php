...
class SpecFile
{
    ...
    {
        info("Generating SDK Files for <fg=yellow>{$this->category}</> / <fg=cyan>{$this->group}</>");
        // info("Parsing Spec File: <fg=yellow>$this->localPath</>");
        $spec = $this->parseSpec();
        // $endpoint = $spec->endpoints[0];
        // Update each of the endpoints with the category and group
        foreach ($spec->endpoints as $endpoint) {
            $endpoint->category = $this->category;
            $endpoint->group = $this->group;
        }

        // dd(array_keys((array)$spec->endpoints[0]));
        // dd($spec->endpoints[0]);
        ...
        // dd(array_keys((array)$code->requestClasses[0]));
    ...
    public function generateSdk(): ?GeneratedCode
    {
        if (!$this->spec) {
        ...
        // $this->code->requestClasses = collect($this->code->requestClasses)
        //     ->map(function (PhpFile $file) {
        //         // $file->getNamespaces()[0]->
        //         // $file->addComment('This file was generated by the Athena SDK Generator.');
        //         // $file->addNamespace('TEST');
        //         return $file;
        //     })
        //     ->toArray();
    ...
    protected static function dumpToFile(PhpFile $file): void
    {
        ...
        info("Writing: <fg=yellow>$outputFullPath</>");
